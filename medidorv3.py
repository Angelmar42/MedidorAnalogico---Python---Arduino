...
class VentanaEjemplo:
   ...
   #     self.conectar = Button(master, text="Conectar", command=self.conectar)
    ...
    #    self.conectar.place(x=50, y=55,width=100, height=30)
        ...
        """canvas = ttk.FigureCanvasTkAgg(plot, master=root)
        
        canvas.get_tk_widget().pack(side=ttk.BOTTOM, fill=ttk.BOTH)
        canvas.draw()
        toolbar = NavigationToolbar2Tk(canvas, root)
        toolbar.update()
        canvas._tkcanvas.pack(side=ttk.TOP, fill=ttk.BOTH, expand=True)"""
        plt.show()
